...
class  ViewHolderParentContestProgram{
    TextView textViewParent;
    ViewHolderParentContestProgram(View v){
        this.textViewParent=v.findViewById(R.id.id_tv_elv_tutorial_parent);
    }

}class  ViewHolderChildContestProgram{
    TextView textViewChild;
    ViewHolderChildContestProgram(View v){
        this.textViewChild=v.findViewById(R.id.id_tv_elv_tutorial_child);
    }

}
public class Adapter_ContestProgram extends BaseExpandableListAdapter {
    Context context;
    //groupName means parents name
    List<String> groupName;
    //1 parent have multiple child
    Map<String,List<String>> childName;
    public Adapter_ContestProgram(Context context){
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        ViewHolderParentContestProgram holderParentContestProgram;
        if(row==null){
            LayoutInflater inflater= (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            row=inflater.inflate(R.layout.layout_elv_tutorial_parents,parent,false);
            holderParentContestProgram=new ViewHolderParentContestProgram(row);
            row.setTag(holderParentContestProgram);
        }
        holderParentContestProgram= (ViewHolderParentContestProgram) row.getTag();
        String value= (String) getGroup(groupPosition);
        holderParentContestProgram.textViewParent.setText(value);
        if (isExpanded){
            holderParentContestProgram.textViewParent.setBackground(context.getResources().getDrawable(R.drawable.bg_elv_tutorial1));
        }else{
            holderParentContestProgram.textViewParent.setBackground(context.getResources().getDrawable(R.drawable.bg_elv_tutorial_parent));
        }
        return row;
    }
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        View row=convertView;
        ViewHolderChildContestProgram holderChildContestProgram;
        if(row==null){
            LayoutInflater inflater= (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            row=inflater.inflate(R.layout.layout_elv_tutorial_child,parent,false);
            holderChildContestProgram=new ViewHolderChildContestProgram(row);
            row.setTag(holderChildContestProgram);
        }
        holderChildContestProgram= (ViewHolderChildContestProgram) row.getTag();
        String value= (String) getChild(groupPosition,childPosition);
        holderChildContestProgram.textViewChild.setText(value);
        return row;
    }
