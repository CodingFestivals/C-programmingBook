...
class  ViewHolderParentOutputProgram{
    TextView textViewParent;
    ViewHolderParentOutputProgram(View v){
        this.textViewParent=v.findViewById(R.id.id_tv_elv_tutorial_parent);
    }

}class  ViewHolderChildOutputProgram{
    TextView textViewChild;
    ViewHolderChildOutputProgram(View v){
        this.textViewChild=v.findViewById(R.id.id_tv_elv_tutorial_child);
    }

}
public class Adapter_OutputProgram extends BaseExpandableListAdapter {
    Context context;
    //groupName means parents name
    List<String> groupName;
    //1 parent have multiple child
    Map<String,List<String>> childName;
    public Adapter_OutputProgram(Context context){
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        ViewHolderParentOutputProgram holderParentOutputProgram;
        if(row==null){
            LayoutInflater inflater= (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            row=inflater.inflate(R.layout.layout_elv_tutorial_parents,parent,false);
            holderParentOutputProgram=new ViewHolderParentOutputProgram(row);
            row.setTag(holderParentOutputProgram);
        }
        holderParentOutputProgram= (ViewHolderParentOutputProgram) row.getTag();
        String value= (String) getGroup(groupPosition);
        holderParentOutputProgram.textViewParent.setText(value);
        if (isExpanded){
            holderParentOutputProgram.textViewParent.setBackground(context.getResources().getDrawable(R.drawable.bg_elv_tutorial1));
        }else{
            holderParentOutputProgram.textViewParent.setBackground(context.getResources().getDrawable(R.drawable.bg_elv_tutorial_parent));
        }
        return row;
    }
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        View row=convertView;
        ViewHolderChildOutputProgram holderChildOutputProgram;
        if(row==null){
            LayoutInflater inflater= (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            row=inflater.inflate(R.layout.layout_elv_tutorial_child,parent,false);
            holderChildOutputProgram=new ViewHolderChildOutputProgram(row);
            row.setTag(holderChildOutputProgram);
        }
        holderChildOutputProgram= (ViewHolderChildOutputProgram) row.getTag();
        String value= (String) getChild(groupPosition,childPosition);
        holderChildOutputProgram.textViewChild.setText(value);
        return row;
    }
