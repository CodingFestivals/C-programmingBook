...
public class Adapter_CTutorial extends BaseExpandableListAdapter {
    Context context;
    //groupName means parents name
    List<String> groupName;
    //1 parent have multiple child
    Map<String,List<String>> childName;
    public Adapter_CTutorial(Context context){
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if(row==null){
            ...
            holderParent=new ViewHolderParent(row);
            row.setTag(holderParent);
        }
        holderParent= (ViewHolderParent) row.getTag();
        String value= (String) getGroup(groupPosition);
        holderParent.textViewParent.setText(value);
        if (isExpanded){
            holderParent.textViewParent.setBackground(context.getResources().getDrawable(R.drawable.bg_elv_tutorial1));
        }else{
            holderParent.textViewParent.setBackground(context.getResources().getDrawable(R.drawable.bg_elv_tutorial_parent));
        }
        return row;
    }
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if(row==null){
            ...
            holderChild=new ViewHolderChild(row);
